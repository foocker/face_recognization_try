...
from models.arcface import Backbone, MobileFaceNet, l2_norm
from losses.face_id_head import Arcface
import torch
...
from utils.utils_arc import get_time, gen_plot, hflip_batch, separate_bn_paras
...
#             self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=40, verbose=True)
            ...
            print("????????", len(self.loader), self.board_loss_every , self.evaluate_every, self.save_every, "||||||")
            # self.agedb_30, self.cfp_fp, self.lfw, self.agedb_30_issame, self.cfp_fp_issame, self.lfw_issame = get_val_data(self.loader.dataset.root.parent)
            # self.val_data, self.val_label = get_val_dataset(conf.val_path)
            self.val_data, self.val_label = get_val_dataset(conf.emore_folder_val)
    ...
    def save_state(self, conf, to_save_folder=False, extra=None, model_only=False):
        ...
        torch.save(
            self.model.state_dict(), save_path /
            ('model_{}_step:{}_{}.pth'.format(get_time(), self.step, extra)))
        if not model_only:
            torch.save(
                self.head.state_dict(), save_path /
            ...
            torch.save(
                self.optimizer.state_dict(), save_path /
                ('optimizer_{}_step:{}_{}.pth'.format(get_time(),self.step, extra)))
    """
...
#         self.writer.add_scalar('{}_far:False Acceptance Ratio'.format(db_name), far, self.step)
    ...
    def train(self, conf, epochs):
        ...
        for e in range(epochs):
            ...
            for imgs, labels in tqdm(iter(self.loader)):
                ...
                if self.step % self.evaluate_every == 0 and self.step != 0:
                    accuracy, best_threshold, roc_curve_tensor = self.evaluate(conf, self.val_data, self.val_label)
                    self.board_val('val_data', accuracy, best_threshold, roc_curve_tensor)
                    """
                ...
                """
                """
                if self.step % evaluate_every == 0 and self.step != 0:
                    self.save_state(conf, accuracy)
                """
                self.step += 1
            if e % 10 == 0 and e != 0:
        ...
        self.save_state(conf, accuracy, to_save_folder=True, extra='_final')
    ...
    def infer(self, conf, faces, target_embs, tta=False):
        ...
        source_embs = torch.cat(embs)    # [n_box, 512]
        # (n_box, 512, 1) - (1, 512, n) = (n_box, 512, n)
        diff = source_embs.unsqueeze(-1) - target_embs.transpose(1,0).unsqueeze(0)     
        # print("shape of embs after:", source_embs.shape, target_embs.shape)
        dist = torch.sum(torch.pow(diff, 2), dim=1)    # (n_box, n)
        minimum, min_idx = torch.min(dist, dim=1)    # (n_box), (n_box)
        print("min_idx before:", min_idx)
        print("threshold:", self.threshold)
        min_idx[minimum > self.threshold] = -1   # if no match, set idx to -1
        print("min match score:", minimum, "\n", "min_idx after:", min_idx)
